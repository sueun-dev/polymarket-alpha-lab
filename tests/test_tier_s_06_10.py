...
def test_s06_scan_disabled_returns_empty():
    s = BTCLatencyArb()
    markets = [
        Market(
            condition_id="0x1",
            question="BTC 15-minute price above $100K?",
            tokens=[{"token_id": "y1", "outcome": "Yes", "price": "0.55"}],
            volume=5000,
        )
    ]
    assert s.scan(markets) == []


def test_s06_has_no_external_data_dependency():
    s = BTCLatencyArb()
    assert s.required_data == []


def test_s06_analyze_disabled_returns_none():
    s = BTCLatencyArb()
...
def test_s07_scan():
    ...
    markets = [
        Market(
            condition_id="0x1",
            question="Bitcoin reserve?",
            tokens=[{"token_id": "y1", "outcome": "Yes", "price": "0.60"}],
            volume=10000,
            description="Resolves YES if government officially announces reserve",
        )
    ]
    opps = s.scan(markets)
    assert len(opps) == 1
